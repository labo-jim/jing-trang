<xsl:stylesheet version="2.0"
  ...
  <!--+==================================================+
      |           Simple 2-steps wrapper for             |
      |     ISO Schematron Skeleton transformation.      |
      +==================================================+-->
  <!-- [ACOU] [20161010] Pas d'URIResolver dans les sources Java, l'import est problématique -->
  ...
  <!-- [ACOU] [20161010] Forcé à true, nécessaire pour autoriser les éléments <xsl:*> dans le Schematron (par exemple) -->
  ...
  <!--+==================================================+
      | MODE = iso-sch-abs                               |
      | FICHIER D'ORIGINE = iso-schematron-abstract.xsl  |
      +==================================================+-->
  
  <xsl:key name="ap" match="iso:pattern[@abstract='true']" use="@id"/>
  
  <xsl:template mode="iso-sch-abs" match="node() | @*">
    <xsl:copy>
      <xsl:apply-templates mode="iso-sch-abs" select="node() | @*"/>
    </xsl:copy>
  </xsl:template>
  
  <xsl:template mode="iso-sch-abs" match="iso:pattern[@abstract='true']"/>
  
  <xsl:template mode="iso-sch-abs" match="iso:pattern[@is-a]">
    <xsl:copy>
  ...
  <!--+==================================================+
      | MODE = iso-sch-ske                               |
      | FICHIER D'ORIGINE = iso_schematron_skeleton.xsl  |
      | IMPORTÉ PAR = iso-schematron-message.xsl         |
      +==================================================+-->
  ...
  <!-- Using XSLT 2 -->
  <xsl:template mode="iso-sch-ske" match="iso:schema[translate(@queryBinding, 'XSLT', 'xslt')='xslt2']" priority="10">
  	<axsl:stylesheet
   ...
   <xsl:template mode="iso-sch-ske" match="iso:include">
       ...
       <xsl:choose> 
          <xsl:when test="string-length( $fragment-id ) != 0">
              <xsl:apply-templates mode="iso-sch-ske" select="document( $document-uri )//iso:*[@id= $fragment-id ]"/>
		   </xsl:when>
		   <xsl:otherwise>
       		<xsl:apply-templates mode="iso-sch-ske" select="document( $document-uri )/*"/>
       	   </xsl:otherwise>
       </xsl:choose>
   </xsl:template>
...
<!-- ============================================================== -->
  ...
  <!-- [ACOU] [20161010] Template issu de iso_schematron_skeleton.xsl, ne convient pas à Jing -->
